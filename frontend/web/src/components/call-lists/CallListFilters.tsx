...
export type AssignmentFilterType = "all" | "assigned" | "unassigned" | "member";
...
export function CallListFilters({
  ...
  const hasActiveFilters = activeFilter !== "all" || assignmentFilter !== "all" || memberFilterId !== null;
  const activeFilterCount = [
    activeFilter !== "all" ? 1 : 0,
    assignmentFilter !== "all" ? 1 : 0,
    memberFilterId !== null ? 1 : 0,
  ].reduce((a, b) => a + b, 0);

  return (
    <div className="space-y-3">
      {/* Status and Assignment Filters on the same row */}
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => onAssignmentFilterChange("member")}
            className={cn(
              "h-7 px-3 text-xs",
      ...
      {/* Member Selector (shown when "By Member" is selected) */}
      {assignmentFilter === "member" && (
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs font-medium text-[var(--groups1-text-secondary)] uppercase tracking-wide">
            Select Member:
          ...
          <div className="w-64">
            <MemberSelector
              selectedMemberId={memberFilterId}
              onSelectMemberId={onMemberFilterChange}
